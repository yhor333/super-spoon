const json = JSON.stringify([
  ...
  },
  {
    id: 3.1,
    question:
      "Have you already known date When you are planning on getting married?",
    answers: [
      {
        answer: "Yes",
        answerId: null,
...
function drawQuestions(json: string) {
  const res: any = [];
    ...
    questionButton.onclick = () => {
      ...
      for (let r of radio) {
        if (r.checked && question !== null) {
          ...
          const currentAnswer = r.parentNode?.textContent;
...
function drawInputs(arr: any, value: number, i: number, res: any) {
  if (value !== 0) {
